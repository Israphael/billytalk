...
| Синхронные Read/Write на одном хэндле канала сериализуются ядром | канал **только `FILE_FLAG_OVERLAPPED`**, оба конца (писатель замер за читателем — цикл 2) |
| `DisconnectNamedPipe` выбрасывает непрочитанный буфер | `FlushFileBuffers` перед разрывом, когда ответ обязан дойти (`protocol_mismatch`) |
| Под uv-венвом `sys.executable` ≠ образ процесса (трамплин) | сверять путь сервера с образом настоящего exe, не с `sys.executable` |
| `CancelIoEx` кромочный: отменяет только уже висящий ввод-вывод | сигнал смерти соединения — уровневое событие в каждом ожидании, не отмена |
| Переполненная исходящая очередь + блокирующий сентинел = вечный клинч | сентинел только `put_nowait` с разгрузкой; поток канала не смеет умирать ни от какого исключения |
